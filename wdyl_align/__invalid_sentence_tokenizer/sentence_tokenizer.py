...
def sentence_break_batch(lines, lang):
    try:
        ...
        for single_line in lines:
            split_lines = tokenizing(single_line, lang)
            all_split_lines.append(split_lines)
        return return_data(0, 'sentence batch break succeed', all_split_lines)
...
def sentence_break_limit(lines, lang, limit=20):
    try:
        ...
        all_split_lines = []
        for single_line in lines:
            split_lines = tokenizing_length_limit(
                single_line, lang, limit)
            all_split_lines.append(split_lines)
        return return_data(0, 'sentence length limit break succeed', all_split_lines)
    except Exception as e:
...
def word_break_limit(lines, lang, limit=20):
    try:
        ...
        all_split_lines = []
        for single_line in lines:
            split_lines = tokenizing_word_limit(
                single_line, lang, limit)
            all_split_lines.append(split_lines)
        return return_data(0, 'sentence word limit break succeed', all_split_lines)
    except Exception as e:
